...
def state_action_space():
    ...
    combs = (current_action, previous_action, previous_success)
...
def simulate(ALPHA, GAMMA, num_interactions, egreedy_param, num_episodes, user_model):
    ...
    while (episode <= episodes):
        ...
        while (not done):
            ...
            reward, next_state = get_next_state(state, states, action, previous_action, user_model)
            if reward > 0: 
                s += 1; 
            next_state_index = states.index(tuple(next_state))
            creturn = (learning.gamma**(interaction-1))*reward
            r += creturn
            ...
            e += abs(error)
            #if (interaction % 499 == 0):
                # print ("Episode: " + str(episode))
            #    print(interaction, state, alabel[action], next_state, reward, egreedy.param, error)
            state = next_state
            previous_action = action
            interaction += 1
            
        episode += 1
        exploration.append(egreedy.param)
        if (episode % eps == 0):
            egreedy.param *= disc  # this can change to suit the problem
        ...
        returns.append(r)
        errors.append(e/float(interactions)) 
        successes.append(s/float(interactions))
    ...
    if user_model ==4:
         with open('q_table_um4_np.py', 'w', newline='') as f:
             ...
             writer.writerows(Q)
    if user_model ==5:
         with open('q_table_um5_np.py', 'w', newline='') as f:
             writer = csv.writer(f,delimiter=' ')
             writer.writerows(Q)         

    return returns, successes, exploration
...
#run1_returns, run1_successes, expl = simulate(ALPHA=0.3, GAMMA=0.9, num_interactions= 180, egreedy_param=0.99, num_episodes=E, user_model=1)
print("user model 1")

#run2_returns, run2_successes, expl = simulate(ALPHA=0.3, GAMMA=0.9, num_interactions= 180, egreedy_param=0.99, num_episodes=E, user_model=2)
print("user model 2")

#run3_returns, run3_successes, expl = simulate(ALPHA=0.3, GAMMA=0.9, num_interactions= 180, egreedy_param=0.99, num_episodes=E, user_model=3)
print("user model 3")

#run4_returns, run4_successes, expl = simulate(ALPHA=0.3, GAMMA=0.9, num_interactions= 180, egreedy_param=0.99, num_episodes=E, user_model=4)
print("user model 4")

run5_returns, run5_successes, expl = simulate(ALPHA=0.3, GAMMA=0.9, num_interactions= 180, egreedy_param=0.99, num_episodes=E, user_model=5)
print("user model 5")
...
plt.legend(['user model 1', 'user model 2', 'user model 3','user model 4', 'user model 5'])
plt.title("Total return")
plt.show()

#plt.plot(moving_average(run1_successes), 'b', moving_average(run2_successes), 'r', moving_average(run3_successes), 'g', moving_average(run4_successes), 'c', moving_average(run5_successes), 'y')
plt.legend(['user model 1', 'user model 2', 'user model 3', 'user model 4', 'user model 5'])
plt.title("Total success rate")
plt.show()

#plt.plot(moving_average(run3_successes))
#plt.show()
#plt.plot(moving_average(run3_returns))
#plt.show()

plt.plot(expl)
plt.legend(["discount " + str(disc) + " per " + str(eps) + " episodes"])
